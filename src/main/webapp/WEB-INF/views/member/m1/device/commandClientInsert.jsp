			...
			$('button').on('click', function() {
				switch ($(this).attr('id')) {
					case "btnReset": // 초기화
	 					break;
					case "btnCancel": // 취소
						WR.callPage('/device/commandClient.do', $('#searchForm').serializeArray());
						break;
					case "btnSave": // 저장
						if(!validateCheck()) {
							return;
						}
						commandClientInsert();	// 디바이스 명령 저장
						break;
				}
			});
...
//					$("#humidifierSetupTimeTwo").attr("disabled", false);
	...
	// 데이터 저장
	function commandClientInsert(){
		var url = "<c:url value='/device/commandClientInsert.do'/>"
		...
		$.post(url, params).done(function(data){
			...
			// 검색 조건 수정
			$("#searchRouter").val($("#eqpmnId").val());
			$("#searchClient").val($("#no").val());
...
//        return date.getFullYear() + '-' + month + '-' + day + ' ' + hour + ':' + minute;
        return date.getFullYear() + '-' + month + '-' + day + ' ';
					...
					<div class="">
			      		<h4 class="page-header"><span class="glyphicon glyphicon-edit"></span> 사용자 <span class="glyphicon glyphicon-chevron-right"></span> 클라이언트 설정 <span class="glyphicon glyphicon-chevron-right"></span> 등록</h4>
						<form id="searchForm" class="">
							<input type="hidden" name="searchRouter" id="searchRouter" value="<c:out value='${sParam.searchRouter}' />">
							<input type="hidden" name="searchClient" id="searchClient" value="<c:out value='${sParam.searchClient}' />">
						...
						<form id="mainForm" class="">
							<div class="panel">
								...
								<div class="form-group form-inline">
									<label for="eqpmnId" class="control-label">라우터</label>
									<select class="form-control" id="eqpmnId" name="eqpmnId" data-role="combo" data-url="/device/routerList.do" data-default="<c:out value='${sParam.searchRouter}' />" data-prompt="선택">
										<option>라우터 선택</option>
									</select>
								</div>
								<div class="form-group form-inline">
									<label for="no" class="control-label">클라이언트</label>
									<select class="form-control" id="no" name="no" data-role="combo" data-url="/device/clientList.do" data-filter='eqpmnId' data-default="" data-prompt="선택">
